...
const SingleFixtureList = ({isLiveGames,isFinishedGames,isScheduledGames,isCancelledGames,isAllGames}:{isLiveGames:boolean,isFinishedGames:boolean,isScheduledGames:boolean,isCancelledGames:boolean,isAllGames:boolean}) => {
  return (
    <div className='relative w-full'>
        <div className={`flex flex-row justify-between items-center bg-white shadow-lg p-3 px-4 rounded-lg border-[#1536c6] border-solid border-[1px] before:content-[''] before:absolute before:w-2 before:h-full before:rounded-l-full before:left-0 before:top-0  ${isLiveGames ? "before:bg-red-600 " : ""} ${isFinishedGames ? "before:bg-green-600 " : ""} ${isScheduledGames ? "before:bg-blue-600 " : ""} ${isCancelledGames ? "before:bg-gray-600 " : ""} ${isAllGames ? "before:bg-gray-600 " : ""}`}> 
            <div className='flex md:items-center flex-col md:flex-row gap-3 md:w-[20%]'>
                <div>
            ...
            <div className='flex md:flex-row flex-col items-center justify-between w-[40%]'>
                <MatchComp/>
            </div>
            <div className='md:flex hidden  items-center justify-end w-[20%]'>
